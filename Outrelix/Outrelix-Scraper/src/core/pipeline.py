...
class Pipeline:
    ...
    async def source_businesses(self) -> Tuple[List[Dict], Dict[str, int]]:
        ...
        # Treat free_mode as “no paid calls”; reuse dry flag for API clients
        ...
        if yelp.dry_run or not self.config.enable_yelp:
            self.mock_warnings.append("yelp_mocked_or_disabled")
        if yellow.dry_run or not self.config.enable_yellowpages:
            self.mock_warnings.append("yellowpages_mocked_or_disabled")
        if overpass.dry_run or not getattr(self.config, "enable_overpass", True):
            self.mock_warnings.append("overpass_mocked_or_disabled")

        results = await asyncio.gather(
            gmaps.search(queries, geo, limit),
            yelp.search(queries, geo, limit) if self.config.enable_yelp else asyncio.sleep(0, result=[]),
            yellow.search(queries, geo, limit) if self.config.enable_yellowpages else asyncio.sleep(0, result=[]),
            overpass.search(queries, geo, limit) if getattr(self.config, "enable_overpass", True) else asyncio.sleep(0, result=[]),
        )
